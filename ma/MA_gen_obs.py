...
if __name__ == '__main__':
    if len(sys.argv) != 5:
        print("Usage: MA_gen_obs.py <seq length> <MC steps> <param filename> <data filename>")
        exit(-1)
    
    seq_length = int(sys.argv[1])
    mc_steps = int(sys.argv[2])
    param_filename = sys.argv[3]
    data_filename = sys.argv[4]

    theta = [[0.6, 0.2]]
    X = MA_gen_batch(1, theta, seq_length)
    AC = [MA_autocov(x) for x in X]
    param_out = np.column_stack((AC, theta))
